...
const hState = { lastPhase: null, data: null, search: "", placeType: "", status: "", logSince: 0, logText: "" };
...
function historySettingsModalHtml(){
  return `
    ...
    <div class="modal-actions">
      <button class="primary" data-action="submit-generate">▶ Generate</button>
    </div>
  `;
}

function openHistorySettingsModal(){
  openModal(historySettingsModalHtml());
  modalBodyEl.querySelector('[data-close]').addEventListener('click', closeModal);
  modalBodyEl.querySelector('[data-action="submit-generate"]').addEventListener('click', () => startHistoryGeneration(false));
}

generateBtn.addEventListener('click', openHistorySettingsModal);

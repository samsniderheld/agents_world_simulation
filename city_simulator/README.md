...
| `history/citymap.py` | history | The city map: a domain-warped-noise archipelago (one main landmass plus organic islets), era neighborhoods carved along noise-wobbled boundary curves, rendered as braille dot-density text plus the structured `graphic` data the web viewer's interactive canvas draws from. |
...
| `citystate/store.py` | citystate | The one persisted "active city" record -- history's generated payload plus per-entity media and agent-simulation runs appended afterward, held in memory and mirrored to `data/city.json` (atomic write, lazy-loaded once per process so a server restart just picks it back up). `history/jobs.py`'s `get_data()`/`_worker` and `agents/simulation.py`'s `run()` read/write through here rather than keeping their own copies. |
| `citystate/routes.py` | citystate | The `/api/city/*` blueprint: `GET /state`, `POST /media` and `DELETE /media/<entity_id>/<media_id>` for attaching/removing a character's or place's media. |
